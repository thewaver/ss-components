...
const bar = (name: string) => `${variant(name)} [role="progressbar"]`;
...
test("a real unit range reaches ARIA unscaled, with a readable text", async ({ page }) => {
    await expect(page.locator(bar("Live range")), "a real unit range reaches ARIA unscaled").toHaveAttribute(
        "aria-valuemax",
        "2400000",
    );
    await expect(
        page.locator(bar("Live range")),
...
test("a value past the end is clamped for the painter but reported as given", async ({ page }) => {
    expect(
        await inlineStyle(page.locator(`${bar("Out of range")} > div > div > div`), "width"),
        "a value past the end reaches the painter as a clamped ratio, not as 5",
    ).toBe("100%");
    await expect(
        page.locator(bar("Out of range")),
        "though ARIA still reports what the owner actually said",
    ).toHaveAttribute("aria-valuenow", "5");
});

test("an errored bar is announced invalid", async ({ page }) => {
    await expect(page.locator(bar("Error")), "an errored bar is announced invalid").toHaveAttribute(
        "aria-invalid",
        "true",
    );
});
...
test("the filling variant is wider than the fit-content one", async ({ page }) => {
    ...
    const fillWidth = await page
        .locator(bar("Filling its container"))
