...
describe("getSegmentRects", () => {
    it("cuts the screen into the eight pieces around the hole", () => {
        expect(Object.keys(ElementHighlightUtils.getSegmentRects(RECT))).toEqual([
            "topLeft",
            "topCenter",
            "topRight",
            "centerLeft",
    ...
    it("leaves the highlighted rect itself uncovered", () => {
        const segments = ElementHighlightUtils.getSegmentRects(RECT);
    ...
    it("collapses the leading segments to nothing for a rect in the top-left corner", () => {
        const segments = ElementHighlightUtils.getSegmentRects({ x: 0, y: 0, width: 40, height: 30 });
