...
export const DEFAULT_SCREENWIPER_SHAPE: ScreenWiperShape = "lozenge";
export const DEFAULT_SCREENWIPER_TRANSITION_DURATION_MS = 100;
export const DEFAULT_SCREENWIPER_CELLSIZE: Size2d = { width: 120, height: 120 };
...
export const ScreenWiper = (props: ScreenWiperProps) => {
    ...
    const [getTarget, setTarget] = createSignal(getTargetFromDirection(props.getFadeDirection()) === 1 ? 0 : 1);
    const [getHasFinished, setHasFinished] = createSignal(false);

    const getCellSize = createMemo(() => props.getCellSize?.() ?? DEFAULT_SCREENWIPER_CELLSIZE);
    ...
    createEffect(() => {
        const direction = props.getFadeDirection();

        setHasFinished(false);
        setTimeout(() => {
            setTarget(getTargetFromDirection(direction));
        }, 0);
    });
                                    ...
                                    {(col) => (
                                        <div
                                            ...
                                            style={{
                                                ...
                                                "transition-delay": `${getTransitionDurationMs() * 0.2 * (col + row)}ms`,
