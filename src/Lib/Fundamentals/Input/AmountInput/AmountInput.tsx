...
const DEFAULT_AMOUNT_INPUT_DECIMALS = 2;
const DEFAULT_AMOUNT_INPUT_GROUP_SIZE = 3;

export const AmountInput = (props: AmountInputProps) => {
    const getDecimals = createMemo(() => props.getDecimals?.() ?? DEFAULT_AMOUNT_INPUT_DECIMALS);

    const getGroupDefs = createMemo((): TextSyncGroupDefs => ({
        ...AmountInputUtils.getSeparators(props.getLocale?.()),
        groupSize: props.getGroupSize?.() ?? DEFAULT_AMOUNT_INPUT_GROUP_SIZE,
    ...
    const fromDigits = (digits: string) => {
        const parsed = AmountInputUtils.fromDigits(digits, getDecimals());
    ...
    const field = MaskedField.createField<number>({
        ...
        toDigits: (value) => AmountInputUtils.toDigits(value, getDecimals()),
        fromDigits,
